...
def oimport(file: str, arg: str):
    # get a specified value of an argument out of the specified file
    try:
        with open(file, 'r') as f:
            for line in f:
                if line.startswith(arg):
                    return line.strip(arg).replace('=', '').strip(' ').strip('"')
    except FileNotFoundError:
        raise MissingRequirements


def write_pubkey():
    with open(std_dir + '.settings/public_key.pem', 'rb') as f:
        with open(_global_path + '.users/pbkeys/' + username + '.pem', 'wb') as fnew:
            fnew.write(f.read())


def make_globalfolder():
    os.mkdir(_global_path + '.chats/' + username)
...
def send_messages(messages):
    ...
    with open(std_dir + 'chats/sent_messages.txt', 'a') as f:
        f.write(text)
    return True
...
def removemessages(messages):
    ...
    os.remove(std_dir + 'chats/sent_messages.txt')
...
def getmessages():
    messages = []
    with open(std_dir + 'chats/sent_messages.txt', 'r') as f:
        for line in f:
...
def add_contact(global_name: str, contact_name: str):
    with open(std_dir + '.settings/usernames.txt') as f:
        for line in f:
            if line.startswith(global_name):
                return
    with open(std_dir + '.settings/usernames.txt', 'a') as f:
...
def read_pub_key(user: str):
    ...
    try:
        pubkey = crypter.load_pubkey(_global_path + '.users/pbkeys/' + user + '.pem')
        return pubkey
    except FileNotFoundError:
...
def read_status(user: str):
    ...
    try:
        with open(_global_path + '.users/status/' + user + '.txt', 'r') as f:
            for line in f:
...
def get_all_chats():
    chatlist = os.listdir(std_dir + 'chats/')
    try:
...
def log(status: str, message: str):
    filepath = std_dir + 'logs/' + status + '.log'
...
std_dir = os.getcwd().replace('\\', '/').rsplit('/', 1)[0] + '/'
username = getusername()
try:
    _global_path = oimport(std_dir + '.settings/path.py', '_global_path')
